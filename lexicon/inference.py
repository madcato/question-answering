...
SRC_VOCAB_SIZE = 1908 # len(vocab_transform[SRC_LANGUAGE])
TGT_VOCAB_SIZE = 3549 # len(vocab_transform[TGT_LANGUAGE])
...
PATH= os.path.join(os.path.dirname(__file__), f'models/', f'transformer0_epoch40.model')
transformer.load_state_dict(torch.load(PATH, map_location=torch.device(DEVICE)))
...
def translate(model: torch.nn.Module, src_sentence: str):
    ...
    tgt_tokens = greedy_decode(
        model,  src, src_mask, max_len=num_tokens + 15, start_symbol=BOS_IDX).flatten()
    return " ".join(vocab_transform[TGT_LANGUAGE].lookup_tokens(list(tgt_tokens.cpu().numpy()))).replace("<bos>", "").replace("<eos>", "")

print(translate(transformer, "sharp"))
