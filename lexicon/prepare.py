...
for type in types:
    src_arr = {}
    src_arr[SRC_LANGUAGE] = []
    src_arr[TGT_LANGUAGE] = []
    dataset = split_dataset[type]
    for split in dataset:
        src = split['text'][0]
        tgt = split['text'][1]
        src_arr[SRC_LANGUAGE].append(src)
        src_arr[TGT_LANGUAGE].append(tgt)

    for language in languages:                     
        filename = os.path.join(os.path.dirname(__file__), f'{type}.{language}')
        print(f'writing {filename}')
        with open(filename, 'w') as file:
            for line in src_arr[language]:
                file.write(f'{line}\n')
